...
MessageQueue::MessageQueue(const int nmsg, const int ntimers, const int npins, const int nvalues, const int npulses)
:consumer_(0)
,producer_(0)
,MAX_MESSAGES(nmsg)
,MAX_TIMERS(ntimers)
,MAX_PINS(npins)
,MAX_VALUES(nvalues+MAX_PINS)
,MAX_PULSES(npulses)
,msg_queue_(new Message[MAX_MESSAGES])
,pins_(new Pin[MAX_PINS])
,pulses_(new Pulse[MAX_PULSES])
,timers_(new Timer[MAX_TIMERS])
,values_(new Value[MAX_VALUES])
, debug_(false)
{
